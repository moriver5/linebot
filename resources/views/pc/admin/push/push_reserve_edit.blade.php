        ...
        <div class="col-md-8 col-md-offset-2">
								...
								<div class="form-group" style="align:center;">
									...
									<table border="1" width="95%">
										<tr style="text-align:center;background:wheat;font-weight:bold;">
											...
											<td>
												<input type="text" id="reserve_date" name="tmp_reserve_date" class="form-control" value="{{ $db_data->reserve_send_date }}" placeholder="">
											</td>
										</tr>
									</table>
										...
										<tr style="text-align:center;background:wheat;font-weight:bold;">
											@if( !empty($list_msg) )
												@foreach($list_msg as $name => $lines)
													@if( preg_match("/^msg\d_\d+\.(png|jpg|jpeg)$/", $lines[0]) > 0 )
													...
													@elseif( preg_match("/^imglinkmsg\d_\d+\.(png|jpg|jpeg)/", $lines[0]) > 0 )
													<tr style="text-align:center;">
														<td colspan="2">
															<div style="width:100%;">
																<div style="width:50%;float:left;">
													...
													<tr style="text-align:center;">
														...
														<table border="1" width="100%">
															<tr style="text-align:center;">
																...
																<input type="hidden" name="url{{ $name }}" value="{{ $after_minute }}">
																<input type="hidden" name="title{{ $name }}" value="{{ $after_minute }}">
																</td>
																<td style="padding:15px;text-align:center;">
																	@if( !empty($db_data) )
			...
			例：11537<>52002735">{{ $lines[0] }}</textarea>
								...
								<form id="formReservePushMessage" class="form-horizontal" method="POST" action="/admin/member/line/push/message/reserve/send">
									...
									<button type="submit" id="emoji" class="btn btn-primary">絵文字</button>
									<button type="submit" id="lineemoji" class="btn btn-primary">LINE絵文字</button>
					...
					<div class="line__container">
						...
						<div class="line__contents scroll">
							...
							@foreach($list_msg as $index => $lines)
								@if( preg_match("/^(msg|imglinkmsg)\d_\d+\.(png|jpg|jpeg)/", $lines[0]) > 0 )
									<!-- 相手のスタンプ -->
...
//		$('#message_form').append("<tr style='text-align:center;'><td><form id='formLinemsg"+idNo+"' class='form-horizontal' method='POST' action='/admin/member/line/push/message/img/{{ $channel_id }}/upload'><div id='file_upload_sectionmsd"+idNo+"' style='width:100%;'><div id='dropmsg"+idNo+"' style='text-align:center;width:1000px;height:130px; vertical-align:middle; display:table-cell;' ondragleave=\"onDragLeave(event, 'dropmsg"+idNo+"', 'white');\" ondragover=\"onDragOver(event, 'dropmsg"+idNo+"', 'wheat');\" ondrop=\"onDrop(event, 'formLinemsg"+idNo+"', 'import_file', '{{csrf_token()}}', '', '{{ __('messages.dialog_upload_error_msg') }}',　['edit_id','msg', 'send_type', 'tmp_reserve_date'], 'post', '10000', '{{ $redirect_url }}', 'msg"+idNo+"');\"><div style='font:italic normal bold 16px/150% メイリオ,sans-serif;color:silver;'>アップロードするファイルをここに<br />ドラッグアンドドロップしてください<br>反映されないときはctrl+F5を押してください</div><input type='hidden' name='page' value='1'><input type='hidden' name='send_type' value='1'><input type='hidden' name='msg' value='msg"+idNo+"'><input type='hidden' name='edit_id' value='{{ $edit_id }}'><input type='hidden' name='tmp_reserve_date' value='{{ $reserve_date }}'><center><div id='resultmsg"+idNo+"' style='font:italic normal bold 16px/150% メイリオ,sans-serif;width:100%;'></div></center></div></div></td><td><button type='submit' id='delmsg"+idNo+"' class='btn btn-primary del_msg' style='margin:5px;'>削除</button></form></td></tr></table>");
...
//		event.preventDefault();
		var reserve_date = $('[name=tmp_reserve_date]').val();
