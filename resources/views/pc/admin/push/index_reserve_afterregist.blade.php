...
$(document).ready(function(){
	...
	$('#add_text_form').click(function(event){
		...
		$('#message_form').append('<tr style="text-align:center;"><td><form id="formLinemsg'+idNo+'" class="form-horizontal" method="POST" action="/admin/member/line/push/message/save/send"><textarea cols="50" rows="7" name="msg'+idNo+'" class="form-control" placeholder="'+idNo+'通目：LINEメッセージの内容"></textarea><input type="hidden" name="page" value="1"><input type="hidden" name="edit_id" value="{{ $edit_id }}"><input type="hidden" name="channel_id" value="{{ $channel_id }}"><input type="hidden" name="send_type" value="4"><input type="hidden" name="msg" value="msg'+idNo+'"><input type="hidden" name="tmp_send_after_minute" value="{{ $reserve_date }}"></td><td><button type="submit" id="delmsg'+idNo+'" class="btn btn-primary del_msg" style="margin:5px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;削除&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</button><br><button type="submit" id="savemsg'+idNo+'" class="btn btn-primary save_msg" style="margin:5px;;">下書き保存</button></form></td></tr>');
