...
-->
				...
				@foreach($db_data as $lines)
					{{-- メルマガ無効のときセル色をグレイ --}}
					@if( !empty($lines->disable) || empty($lines->follow_flg) )
					...
					@endif
						<td style="padding:2px;text-align:center;">
							<a href="{{ url('/admin/member/client/list/') }}/{{ $db_data->currentPage() }}/{{$lines->user_line_id}}">{{ $lines->id }}</a>
						</td>
						<td style="padding:2px;text-align:center;">
							{{ $lines->line_basic_id }}
						</td>
						...
						<td style="padding:2px;text-align:center;">
							@if( empty($lines->follow_flg) )
								<span style="color:red;font-weight:bold;">解除</span>
							@else
								〇
							@endif
						</td>
