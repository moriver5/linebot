...
class LineReserve2ChoicesBroadcast extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'line:reserve_2choices';
     ...
     * @var string
     */
    protected $description = '友達登録しているユーザーのLINEへ2択(確認)テンプレートを予約配信';
    ...
    public function handle()
    {
		...
		$db_data = Line_2choices_template::query()
		...
		if( !empty($db_data) ){
			foreach($db_data as $lines){
				//別プロセスでメルマガIDごとに配信
				$process = new Process(config('const.artisan_command_path')." line:reserve_2choices_broadcast_delivery  {$lines->id} > /dev/null");
