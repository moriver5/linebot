...
use App\Http\Controllers\Api\Line4ChoicesPushMessageController;
...
class LineReserve4ChoicesBroadcastDelivery extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'line:reserve_4choices_broadcast_delivery {line_push_id}';
    ...
    {
		$list_db_choices = Line_4choices_template::join('line_4choices_logs', 'line_4choices_logs.master_id', '=', 'line_4choices_templates.id')->where('line_4choices_templates.id', $this->argument('line_push_id'))->first();
		...
		if( !empty($db_user_data) ){
			...
			//メルマガ配信日時 配信状況：1(送信中)
			$update = Line_4choices_template::where('id', $this->argument('line_push_id'))
				->update([
					'send_date' => $now_date,
					'send_status' => 1]);
			...
			$choices_obj = new Line4ChoicesPushMessageController();

			//LINEメッセージ配信
			$choices_obj->sendLine4ChoicesTemplate($bot, $list_db_choices->line_basic_id, $this->argument('line_push_id'), $list_line_id, $list_db_choices);

			//履歴を残す以外(send_status:4以外)
			//メルマガ配信状況の更新→配信済:send_status→2
			$update = Line_4choices_template::where('id', $this->argument('line_push_id'))->update(['send_status' => 2, 'send_count' => count($list_line_id)]);
		}
    }
}
