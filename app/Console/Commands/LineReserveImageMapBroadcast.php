...
class LineReserveImageMapBroadcast extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'line:reserve_imagemap';
    ...
    public function handle()
    {
		...
		$db_data = Line_imagemap_log::query()
		...
		if( !empty($db_data) ){
			foreach($db_data as $lines){
				//別プロセスでメルマガIDごとに配信
				$process = new Process(config('const.artisan_command_path')." line:reserve_imagemap_broadcast_delivery  {$lines->id} > /dev/null");
