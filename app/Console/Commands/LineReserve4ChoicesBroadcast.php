...
class LineReserve4ChoicesBroadcast extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'line:reserve_4choices';
    ...
    public function handle()
    {
		...
		$db_data = Line_4choices_template::query()
		...
		if( !empty($db_data) ){
			foreach($db_data as $lines){
				//別プロセスでメルマガIDごとに配信
				$process = new Process(config('const.artisan_command_path')." line:reserve_4choices_broadcast_delivery  {$lines->id} > /dev/null");
